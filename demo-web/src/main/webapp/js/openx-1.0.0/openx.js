...
function OpenxClient() {
    ...
    var _default_timeout = 9 * 1000; // default timeout time in ms
    ...
    this.invoke = function (url, type, data, timeout, onResult, onError) {
        // conf general parameter
        ...
        if (!onError) {
            onError = function (json) {
                alert('default onError:' + json);
            }
        }

        // switch to app or jsonp
        if (type == 'html' || type == 'app') {
            this.invokeByPost(url, type, data, time_out, onResult, onError);
        } else if (type == 'jsonp') {
            this.invokeByJsonp(url, type, data, time_out, onResult, onError);
        } else {
            alert('Unknown invoke type ' + type);
        }
    };

    this.invokeByPost = function (url, type, data, timeout, onResult, onError) {
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                onError(xhr.responseText, xhr.status, error);
            }
        });
    };

    this.invokeByJsonp = function (url, type, data, timeout, onResult, onError) {
        data._openx_head = _self.head();
        var json = $.toJSON(data);
        ...
        $.ajax({
            timeout: timeout,
            url: url + '?code=jsonp&data=' + bs64,
            type: 'get',
            dataType: "jsonp",
            contentType: "application/json",
            success: function (data) {
                var head = data._openx_head;
                if (head && head.status && head.status == 'error') {
                    onError(data.data, 500, 'error');
                } else {
                    onResult(data);
                }
            },
